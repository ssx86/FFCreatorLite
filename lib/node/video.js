...
class FFVideo extends FFImage {
  constructor(conf = { x: 0, y: 0, animations: [] }) {
    ...
    const { appearTime, duration } = conf;
    this.setAppearTime(appearTime);
    this.setDuration(duration);
    this.addFilter(`setpts=PTS-STARTPTS+${appearTime}/TB`);
  }
  ...
  addInput(command) {
    ...
    if (clipStartTime && clipEndTime) {
      command.inputOption('-ss', clipStartTime).inputOption('-t', clipEndTime - clipStartTime);
    }
